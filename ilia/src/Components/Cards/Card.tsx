...
function Card(props: CardProps) {
	...
	return (
		<Link href={`/costumer/${costumer.id}`}>
			<div className="flex flex-col justify-center items-center bg-green-300 w-80 gap-3 h-44 rounded-md transform duration-150 hover:scale-110 hover:cursor-pointer">
				<div>Costumer name: {costumer.name}</div>
				<div>Costumer email: {costumer.email}</div>
				<div>Created at: {formatedDate}</div>
				<Link href={`/costumer/${costumer.id}`}>
					<div className="font-bold text-blue-500">See more...</div>
				</Link>
			</div>
		</Link>
	);
